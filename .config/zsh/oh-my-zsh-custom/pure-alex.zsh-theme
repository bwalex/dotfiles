...
function prompt_pure_check_jobs {
	...
	if [[ $#js -gt 0 ]]; then
		prompt_pure_jobs=" %F{yellow}[${(j:,:)js}]%f"
	fi
}

function prompt_pure_check_rbenv {
	prompt_pure_rbenv=""

	if hash rbenv 2> /dev/null; then
		prompt_pure_rbenv=" rbenv:%F{magenta}$(rbenv version-name)%f"
	fi
}
...
prompt_pure_precmd() {
	...
	# check rbenv version(s)
	prompt_pure_check_rbenv

	# check for background jobs
	prompt_pure_check_jobs
...
prompt_pure_preprompt_render() {
	...
	# rbenv if applicable
	preprompt+=$prompt_pure_rbenv
	# background jobs if applicable
	preprompt+=$prompt_pure_jobs
...
prompt_pure_setup() {
	...
	[[ $UID -eq 0 ]] && prompt_pure_username='%f%n%F{cyan}@%f%B%m%f%b '

	# prompt turns red if the previous command didn't exit with 0
	# XXX: prompt used to be magenta... but cyan really is nicer
