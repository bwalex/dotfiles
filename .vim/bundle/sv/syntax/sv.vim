" Vim syntax file
" Language:	SV
" Maintainer:	David Larson (opine at bluebottle dOt com)
" Last Change:	Sep 30, 2011
...
syn keyword     svAssert        intersect first_match throughout within property endproperty
syn keyword     svCoverage      coverpoint cross wildcard bins ignore_bins illegal_bins
syn keyword     svSpecify       pulsestyle_onevent pulsestyle_ondetect
syn keyword     svSpecify       noshowcancelled showcancelled ifnone
syn keyword     svTime          timeunit timeprecision s ms ns us ns ps fs step

syn keyword	svLabel	bad_state bad_trans bind constraint cosvge_group modport
syn keyword	svLabel	class endclass CLOCK default endfunction function interface endinterface m_bad_state
syn keyword	svLabel	m_bad_trans m_state m_trans randseq state task endtask
syn keyword	svLabel	trans begin end solve design instance cell liblist use clocking endclocking
syn keyword	svLabel	covergroup endgroup program endprogram endcase module endmodule
syn keyword	svLabel	package endpackage sequence endsequence checker endchecker
syn keyword	svLabel	import

syn keyword	svConditional	if else case casex casez randcase unique priority matches
syn keyword 	svRepeat        repeat while for do foreach forever
syn keyword 	svModifier	after all any around assoc_size async forkjoin
syn keyword 	svModifier	before big_endian bit_normal bit_reverse export
syn keyword 	svModifier	extends endextends extern little_endian local hdl_node hdl_task
syn keyword 	svModifier	negedge none packed protected posedge rules
syn keyword 	svModifier	shadow soft static super this typedef unpacked var
syn keyword 	svModifier	vca virtual virtuals wildcard edge wait wait_order pure

syn keyword 	svType	reg string enum event bit logic byte
syn keyword 	svType	rand randc integer port prod coverpoint
...
syn keyword 	svTask	alloc assoc_index cast cm_cosvge
syn keyword 	svTask	cm_get_cosvge cm_get_limit delay error error_mode
...
syn keyword 	svTask	urandom urandom_range sv_bit_reverse sv_crc
syn keyword 	svTask	sv_pack sv_pack_big_endian sv_plot
syn keyword 	svTask	sv_report_profile sv_unpack sv_unpack_big_endian
...
syn match	svOperator	"[&|\~><!*@+/=,.\^\-]"
syn keyword	svOperator	or in dist not inside with

" open sv class methods
syn keyword	svMethods	atobin atohex atoi atooct backref bittostr capacity
syn keyword	svMethods	compare Configure constraint_mode delete DisableTrigger
syn keyword	svMethods	DoAction empty EnableCount EnableTrigger Event find
syn keyword	svMethods	find_index first first_index GetAssert get_at_least
syn keyword	svMethods	get_auto_bin getc GetCount get_cosvge_goal get_cov_weight
syn keyword	svMethods	get_cross_bin_max GetFirstAssert GetName GetNextAssert
syn keyword	svMethods	hide hash icompare insert
syn keyword	svMethods	inst_get_at_least inst_get_auto_bin_max inst_get_collect
syn keyword	svMethods	inst_get_cosvge_goal inst_get_cov_weight inst_getcross_bin_max
syn keyword	svMethods	inst_query inst_set_at_least inst_set_auto_bin_max
syn keyword	svMethods	inst_set_bin_activiation inst_set_collect inst_set_cosvge_goal
syn keyword	svMethods	inst_set_cov_weight inst_set_cross_bin_max itoa last last_index
...
syn keyword	svMethods	set_cosvge_goal set_cov_weight set_cross_bin_max set_name
...
syn match	svFormat	display "%\(\d\+\$\)\=[-+' #0*]*\(\d*\|\*\|\*\d\+\$\)\(\.\(\d*\|\*\|\*\d\+\$\)\)\=\([hlL]\|ll\)\=\([bdiuotxXhDOUfeEgGcCsSpnm]\|\[\^\=.[^]]*\]\)" contained
...
syn keyword svConstant	NAME NE NEGEDGE NEXT NO_OVERLAP NO_OVERLAP_STATE
...
syn match	svInclude	display "^\s*\(`\|#\)\s*incdir\>\s*["<]" contains=svIncluded
syn match	svInclude	display "^\s*\(`\|#\)\s*library\>\s*["<]" contains=svIncluded
...
syn keyword     svDefine        timescale
...
if version >= 508 || !exists("did_sv_syn_inits")
  if version < 508
    let did_sv_syn_inits = 1
